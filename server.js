...
function getEntityId(body) {
    ...
    if (body.leave?.id)
        return body.leave.id.toString();
    if (body.user?.id)
        return body.user.id.toString();
    if (body.team?.id)
        return body.team.id.toString();
    return null;
}
...
function getCategoryFromUrl(url) {
    ...
    if (url.includes("/api/admin/roles"))
        return "PERMISSION";
    if (url.includes("/api/admin/team-members"))
        return "TEAM";
    if (url.includes("/api/leaves"))
        return "LEAVE";
...
// ============================================================
async function logAction(req, actionType, category, entityId = null, entityName = null, changes = null, status = "success", errorMessage = null) {
    try {
...
app.use(async (req, res, next) => {
    ...
    res.json = function (body) {
        ...
        if (isWriteOperation && !hasLogged) {
            ...
            logAction(req, actionType, category, entityId || null, entityName || "unknown", changes, logStatus, errorMessage).catch((err) => console.error("❌ [MIDDLEWARE] Błąd logowania:", err));
        }
...
// backend/src/server.ts
app.get("/api/auth/onboarding-status", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.get("/api/ideas", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        const mappedIdeas = ideas.map((idea) => {
            const upvotes = idea.votes.filter((v) => v.vote_type === "up").length;
            const downvotes = idea.votes.filter((v) => v.vote_type === "down").length;
            const userVote = userId
                ? idea.votes.find((v) => v.user_id === userId)
                : null;
            return {
                id: idea.id.toString(),
                title: idea.title,
...
// 📤 POST - dodaj pomysł
// 📤 POST - dodaj pomysł
app.post("/api/ideas", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
// 📤 POST - głosuj na pomysł
// 📤 POST - głosuj na pomysł
app.post("/api/ideas/:id/vote", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        const { type } = req.body;
...
// ✅ FUNKCJA POMOCNICZA
async function getVoteCounts(ideaId) {
...
app.put("/api/ideas/:id/status", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get("/api/ideas/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const idea = await prisma.idea.findUnique({
            where: { id: parseInt(id) },
...
app.get("/api/members", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const mappedUsers = users.map((user) => {
            ...
            const activeLeave = user.leaves?.find((leave) => {
...
// PROJEKTY
app.get("/api/projects", auth_middleware_1.authMiddleware, projectController.getAllProjects);
app.get("/api/projects/:id", auth_middleware_1.authMiddleware, projectController.getProjectById);
app.post("/api/projects", auth_middleware_1.authMiddleware, projectController.createProject);
app.put("/api/projects/:id", auth_middleware_1.authMiddleware, projectController.updateProject);
app.delete("/api/projects/:id", auth_middleware_1.authMiddleware, projectController.deleteProject);
// UŻYTKOWNICY
app.get("/api/users", auth_middleware_1.authMiddleware, userController.getAllUsers);
// 📥 GET - pobierz wszystkie zespoły
app.get("/api/teams", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.post("/api/teams", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { name, role, description, icon, status, parent_id, email } = req.body;
        if (!name) {
...
// STATYSTYKI
app.get("/api/dashboard/stats", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.get("/api/dashboard/notifications", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        const userRole = req.user?.role;
        if (!userId) {
...
app.put("/api/dashboard/notifications/:id/read", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
...
app.put("/api/dashboard/notifications/read-all", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: "Brak autoryzacji" });
...
// 📥 GET - pobierz wszystkie poradniki
app.get("/api/tutorials", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const tutorials = await prisma.guide.findMany({
            where: { is_published: 1 },
            orderBy: { created_at: "desc" },
        });
        const mappedTutorials = tutorials.map((t) => {
            ...
            try {
                ...
                console.warn(`⚠️ Nie udało się sparsować attachments dla tutorial ${t.id}:`, e);
                attachments = [];
            }
            try {
                ...
                else if (t.functional_roles &&
                    typeof t.functional_roles === "object") {
                    functionalRoles = t.functional_roles; // już jest obiektem
                }
            }
            catch (e) {
                console.warn(`⚠️ Nie udało się sparsować functional_roles dla tutorial ${t.id}:`, e);
                functionalRoles = [];
            }
            return {
                id: t.id.toString(),
...
app.post("/api/tutorials", auth_middleware_1.authMiddleware, upload.array("files", 5), async (req, res) => {
    try {
...
app.get("/api/applications", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        const userRole = req.user?.role;
        ...
        console.log("📥 Pobieranie zgłoszeń dla użytkownika:", userId, "rola:", userRole);
...
// ============================================================
app.post("/api/vacancies/:id/notify", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get("/api/vacancies/:id/applications", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        const userRole = req.user?.role;
        ...
        const mappedApplications = applications.map((app) => {
            ...
            if (app.answers && app.answers.length > 0) {
                app.answers.forEach((answer) => {
                    const questionId = answer.question_id?.toString() || answer.id?.toString();
                    if (questionId) {
                        answers[questionId] = answer.answer || "";
                    }
                });
...
app.put("/api/applications/:id/status", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { status } = req.body;
        const userRole = req.user?.role;
...
app.put("/api/tutorials/:id", auth_middleware_1.authMiddleware, upload.array("files", 5), async (req, res) => {
    try {
        const id = parseInt(req.params.id);
...
app.delete("/api/tutorials/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
...
app.delete("/api/tutorials/attachments/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const tutorialId = parseInt(req.query.tutorialId);
        ...
        if (attachmentToRemove) {
            const filePath = path_1.default.join(__dirname, attachmentToRemove.url);
            if (fs_1.default.existsSync(filePath)) {
                fs_1.default.unlinkSync(filePath);
            }
...
// 📥 GET - pobierz plik
// 📥 GET - pobierz plik
app.get("/api/uploads/tutorials/:filename", async (req, res) => {
    try {
...
app.get("/api/vacancies", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.post("/api/vacancies", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { title, icon, description, responsibilities, requirements, nice_to_have, team, team_id, pillar, contact_person_id, status, recruitment_type, recruitment_deadline, recruitment_form_url, recruitment_messenger_contact, questions, } = req.body;
...
app.put("/api/vacancies/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { title, icon, description, responsibilities, requirements, nice_to_have, team, team_id, pillar, contact_person_id, status, recruitment_type, recruitment_deadline, recruitment_form_url, recruitment_messenger_contact, questions, } = req.body;
...
app.delete("/api/vacancies/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// 📤 POST - zgłoś się na wakat
app.post("/api/vacancies/:id/apply", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
...
app.get("/api/vacancies/:id/check-application", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        if (!userId) {
...
// POWIADOMIENIA - USUŃ
app.delete("/api/dashboard/notifications/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        const id = parseInt(req.params.id);
        if (!userId) {
...
// STRUKTURA
// STRUKTURA
app.get("/api/structure", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const teamMap = {};
        teams.forEach((team) => {
            const sortedPeople = (peopleByTeam[team.id] || []).sort((a, b) => {
...
app.use("/api", member_routes_1.default);
// ============================================================
// PROFIL UŻYTKOWNIKA
// ============================================================
...
// 📥 GET - pobierz profil użytkownika (swój lub innego)
// 📥 GET - pobierz profil użytkownika (swój lub innego)
app.get("/api/profile", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        const onboarding = user.onboarding_data?.[0] || {};
        // ✅ FILARY - wszystkie
        const pillars = teams.filter((t) => t.includes("Filar"));
        ...
        const profile = {
            ...
            joinDate: user.join_date?.toISOString().split("T")[0] ||
                user.created_at.toISOString().split("T")[0],
            currentTasks: [],
            projects: [],
...
app.get("/api/leaves", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        const userRole = req.user?.role;
...
// 📤 POST - utwórz nowy wniosek urlopowy
// 📤 POST - utwórz nowy wniosek urlopowy
app.post("/api/leaves", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Brak autoryzacji" });
        console.log("📥 OTRZYMANE DANE:", req.body);
        const { type, scope, affectedTeams, startDate, endDate, reason, reasonVisibility, attachments, } = req.body;
...
// 📝 PUT - aktualizuj wniosek urlopowy
app.put("/api/leaves/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        const userRole = req.user?.role;
        ...
        const canApprove = userRole === "admin" || userRole === "board" || userRole === "zarząd";
        if (!canApprove) {
            return res.status(403).json({
                error: "Tylko Admin lub Zarząd może zatwierdzać lub odrzucać wnioski urlopowe",
            });
        }
        ...
        const { type, scope, affectedTeams, startDate, endDate, reason, reasonVisibility, attachments, status, } = req.body;
...
app.delete("/api/leaves/:id", auth_middleware_1.authMiddleware, async (req, res) => {
...
app.get("/api/leaves/status", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Brak autoryzacji" });
...
app.get("/api/leaves/status/:userId", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.put("/api/profile", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: "Brak autoryzacji" });
...
app.post("/api/profile/skills", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
...
app.delete("/api/profile/skills/:skill", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
...
// ============================================================
// ---------------------- MEMBERS ----------------------
app.get("/api/social/members", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.get("/api/social/creators", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.post("/api/social/creators", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
// ---------------------- PUBLICATIONS ----------------------
app.get("/api/social/publications", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
// ---------------------- MATERIALS ----------------------
app.get("/api/social/materials", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
// ---------------------- TASKS ----------------------
app.get("/api/social/tasks", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
// ---------------------- CONTACTS ----------------------
app.get("/api/social/contacts", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
// ---------------------- MEMBERS ----------------------
// POST - Dodaj członka
app.post("/api/social/members", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.post("/api/social/publications", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.put("/api/social/publications/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { title, platform, type, responsible_id, due_date, status, description, } = req.body;
...
app.delete("/api/social/publications/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// ONBOARDING - ZAPIS DANYCH
// ============================================================
app.post("/api/onboarding/save", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: "Brak autoryzacji" });
        ...
        const { firstName, lastName, email, phone, province, joinDate, isTrial, developmentAreas, skills, experience, availability, description, salaContacts, mpContacts, institutionContacts, otherContacts, pillarIds, } = req.body;
        ...
        if (pillarIds && Array.isArray(pillarIds) && pillarIds.length > 0) {
            ...
            const validPillarIds = pillarIds.filter((id) => existingTeamIds.includes(id));
            ...
            if (validPillarIds.length > 0) {
                ...
                const existingMembershipIds = existingMemberships.map((m) => m.team_id);
                console.log("🔍 [ONBOARDING] Istniejące członkostwa:", existingMembershipIds);
                // Dodaj tylko te, których jeszcze nie ma
                const toAdd = validPillarIds.filter((id) => !existingMembershipIds.includes(id));
        ...
        try {
            ...
            if (existingNotification) {
                console.log(`⏭️ [ONBOARDING] Powiadomienie już istnieje dla użytkownika ${userId}`);
            }
            else {
                ...
                console.log(`✅ [ONBOARDING] Utworzono powiadomienie ID: ${notification.id} dla użytkownika ${userId}`);
            }
        }
        catch (notificationError) {
            // ⚠️ NIE BLOKUJ PROCESU - powiadomienie to tylko dodatek
            console.error("⚠️ [ONBOARDING] Błąd tworzenia powiadomienia:", notificationError);
...
// ---------------------- MATERIALS ----------------------
// POST - Dodaj materiał
app.post("/api/social/materials", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.put("/api/social/materials/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, responsible_id, deadline, priority, stage } = req.body;
        if (!name || !responsible_id || !deadline) {
...
app.delete("/api/social/materials/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// ---------------------- TASKS ----------------------
// POST - Dodaj zadanie
app.post("/api/social/tasks", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.put("/api/social/tasks/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, responsible_id, deadline, status } = req.body;
...
// DELETE - Usuń zadanie
app.delete("/api/social/tasks/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
// ---------------------- CONTACTS ----------------------
// POST - Dodaj kontakt
app.post("/api/social/contacts", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.put("/api/social/contacts/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, channel, responsible_id, email, phone, notes } = req.body;
...
// DELETE - Usuń kontakt
app.delete("/api/social/contacts/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get("/api/admin/permissions/:role", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
...
app.get("/api/admin/roles", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
...
app.put("/api/admin/roles/:id/permissions", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
...
app.post("/api/admin/roles", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
...
app.delete("/api/admin/roles/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
        const roleId = parseInt(req.params.id);
        if (userRole !== "admin") {
...
app.get("/api/admin/teams", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
        // Tylko admin
        if (userRole !== "admin") {
...
// 📝 PUT - edytuj zespół
app.put("/api/admin/teams/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
...
app.post("/api/admin/teams", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
        if (userRole !== "admin") {
            return res.status(403).json({ error: "Brak uprawnień" });
...
app.delete("/api/admin/teams/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
        const teamId = parseInt(req.params.id);
        if (userRole !== "admin") {
...
app.post("/api/admin/team-members", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
        if (userRole !== "admin") {
            return res.status(403).json({ error: "Brak uprawnień" });
...
app.delete("/api/admin/team-members/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
...
app.put("/api/admin/team-members/:id", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
        const memberId = parseInt(req.params.id);
        if (userRole !== "admin") {
...
app.get("/api/admin/available-users", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
        if (userRole !== "admin") {
            return res.status(403).json({ error: "Brak uprawnień" });
        ...
        const formattedUsers = users.map((user) => ({
            ...
            province: user.province || "",
            team_ids: user.team_members.map((tm) => tm.team_id.toString()),
        }));
...
app.get("/api/admin/logs", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userRole = req.user?.role;
...
app.listen(port, () => {
    ...
    console.log(`📋 Dostępne modele:`, Object.keys(prisma).filter((key) => !key.startsWith("_")));
});
setTimeout(async () => {
