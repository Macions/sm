# ============================================
# SKRYPT SPRAWDZAJÄ„CY WSZYSTKIE PLIKI
# ============================================
...
# ============================================
Write-Host "[5] SPRAWDZANIE NODE_MODULES..." -ForegroundColor Yellow
Write-Host ""

if (Test-Path "node_modules") {
    $count = (Get-ChildItem "node_modules" -Directory -ErrorAction SilentlyContinue).Count
    Write-Host "  [OK] node_modules istnieje ($count modulow)" -ForegroundColor Green
} else {
    Write-Host "  [X] Brak node_modules - uruchom: npm install" -ForegroundColor Red
    $allGood = $false
}
Write-Host ""
