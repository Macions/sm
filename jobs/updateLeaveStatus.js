    ...
    try {
        ...
        for (const user of usersOnLeave) {
            if (user.status !== "vacation") {
                ...
                console.log(`✅ ${updatedUser.first_name} ${updatedUser.last_name}: ${user.status} -> vacation (zapisano poprzedni: ${user.status})`);
            }
            else {
                console.log(`ℹ️ ${user.first_name} ${user.last_name} już ma status vacation (poprzedni: ${user.previous_status || 'brak'})`);
            }
        }
        ...
        for (const user of usersNotOnLeave) {
            ...
            console.log(`✅ ${updatedUser.first_name} ${updatedUser.last_name}: vacation -> ${previousStatus} (przywrócono)`);
        }
        console.log(`📊 [JOB] Zakończono: ${usersOnLeave.length} na urlopie, ${usersNotOnLeave.length} wróciło`);
    }
    catch (error) {
